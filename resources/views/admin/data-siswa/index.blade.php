...
<head>
  <meta charset="UTF-8" />
  <title>Data Siswa | BK Care SMK Budi Bakti Ciwidey</title>
  ...
  <div id="mobileSidebar" class="sidebar-mobile">
    <div class="h-full w-full glass-card-deep rounded-r-2xl p-5 flex flex-col overflow-y-auto border-r border-white/40 shadow-2xl">
      ...
      <nav class="flex-1 overflow-y-auto py-5 space-y-1 text-sm">
        <a href="{{ route('admin.dashboard') }}" class="menu-item flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 hover:bg-[#4F8DB8]/10 transition-all">
        ...
        <div class="space-y-1">
          <button onclick="toggleMobileMenu('dataMasterMobile')" 
            ...
            <i class="fas fa-chevron-down text-xs text-white transition-transform duration-300" id="chevron-dataMasterMobile"></i>
          </button>
          <div id="dataMasterMobile" class="ml-6 mt-1 space-y-1">
            <a href="{{ route('data-guru.index') }}" class="submenu-item flex items-center gap-3 px-4 py-2.5 rounded-xl text-gray-600 hover:bg-[#4F8DB8]/10 transition-all pl-9">
              <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
              <span class="text-sm">Data Guru BK</span>
            </a>
            <a href="{{ route('data-wali-kelas.index') }}" class="submenu-item flex items-center gap-3 px-4 py-2.5 rounded-xl text-gray-600 hover:bg-[#4F8DB8]/10 transition-all pl-9">
              <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
              <span class="text-sm">Data Wali Kelas</span>
            </a>
            <a href="{{ route('data-siswa.index') }}" class="submenu-item flex items-center gap-3 px-4 py-2.5 rounded-xl bg-[#4F8DB8]/10 text-[#4F8DB8] font-medium pl-9">
              <span class="w-1 h-1 bg-[#4F8DB8] rounded-full"></span>
              <span class="text-sm">Data Siswa</span>
              <span class="ml-auto bg-[#4F8DB8]/10 text-[#4F8DB8] text-[10px] px-1.5 py-0.5 rounded-full">{{ $totalSiswa }}</span>
            </a>
          </div>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto py-5 space-y-1 text-sm">
          ...
          <div class="space-y-1">
            <button onclick="toggleMenu('dataMaster')" 
              ...
              <i class="fas fa-chevron-down text-xs text-white transition-transform duration-300" id="chevron-dataMaster"></i>
            </button>
    ...
    <main class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 relative z-10 lg:ml-0">
      ...
      <div class="glass-card-deep rounded-xl md:rounded-2xl p-3 md:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-white/40 shadow-xl">
        <div class="flex items-center gap-2 md:gap-3">
          ...
          <div>
            ...
            <p class="text-[10px] md:text-xs text-gray-500 flex items-center gap-1 md:gap-2">
              ...
              <span>Data Siswa SMK Budi Bakti Ciwidey</span>
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 text-xs text-gray-400 self-end sm:self-auto">
          ...
          <i class="fa-solid fa-chevron-right text-[10px]"></i>
          <span class="text-[#4F8DB8] font-medium">Data Siswa</span>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <!-- INFO CARD -->
        <div class="glass-card-deep rounded-xl p-4 flex-1 flex items-center gap-4 border border-white/40 shadow-md">
          <div class="w-12 h-12 bg-[#17235b]/10 rounded-xl flex items-center justify-center">
            <i class="fa-solid fa-user-graduate text-[#17235b] text-xl"></i>
          </div>
          <div>
            <h3 class="font-semibold text-[#17235b]">Total Siswa Aktif</h3>
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold text-[#17235b]">{{ $totalSiswa }}</span>
              <span class="text-xs text-gray-500">siswa</span>
              <span class="ml-2 text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">+12 dari bulan lalu</span>
            </div>
          </div>
        </div>

  <a
  href="{{ route('data-siswa.create') }}"
  class="flex items-center gap-2 px-6 py-3.5
         bg-gradient-to-r from-[#4F8DB8] to-[#6FAED9]
         text-white text-sm font-semibold rounded-xl
         hover:shadow-lg hover:shadow-[#4F8DB8]/30
  ...
  Tambah Siswa Baru
      ...
      <div class="glass-card-deep rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/40 shadow-xl">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <!-- SEARCH -->
      <form method="GET" action="{{ route('data-siswa.index') }}" class="relative flex-1 max-w-md">
...
<form method="GET" action="{{ route('data-siswa.index') }}"
  ...
  <select name="kelas"
    class="px-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#4F8DB8]">
  ...
  <select name="jurusan"
    ...
    <option value="BDP" {{ request('jurusan') == 'BDP' ? 'selected' : '' }}>BDP</option>
    <option value="DKV" {{ request('jurusan') == 'DKV' ? 'selected' : '' }}>DKV</option>
  </select>

  <button type="submit"
    class="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm text-gray-700 flex items-center gap-2">
    ...
    Filter
          ...
          <table class="w-full text-sm border-separate border-spacing-0">
            <thead>
              <tr class="bg-gradient-to-r from-[#4F8DB8]/10 to-[#6FAED9]/10">
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#17235b] uppercase tracking-wider">NIPD</th>
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#17235b] uppercase tracking-wider">Email</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#17235b] uppercase tracking-wider">No HP</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#17235b] uppercase tracking-wider">Alamat</th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-[#17235b] uppercase tracking-wider">JK</th>
            ...
            <tbody class="divide-y divide-gray-200">
              ...
              <tr class="hover:bg-[#4F8DB8]/5 transition-colors group">
                ...
                <td class="px-4 py-3 text-sm text-gray-600">{{$item->nipd}}</td>
                <td class="px-4 py-3">
                  <span class="px-2 py-1 bg-[#4F8DB8]/10 text-[#4F8DB8] text-xs rounded-full font-medium">{{$item->kelas}}</span>
                </td>
                <td class="px-4 py-3 text-sm text-gray-600">{{$item->jurusan}}</td>
                ...
                <td class="px-4 py-3">
                  <span class="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{{$item->jk}}</span>
                </td>
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
              {{-- detail --}}
            ...
            <i class="fa-regular fa-pen-to-square"></i>
              ...
              <form id="delete-form-{{ $item->id }}"
                    action="{{ route('data-siswa.destroy', $item->id) }}"
                    method="POST"
                    class="inline">
                  @csrf
                  @method('DELETE')
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-4 border-t border-[#4F8DB8]/20">
          ...
          <div class="text-sm text-gray-500">
            Menampilkan <span class="font-medium text-[#4F8DB8]">1-{{ $siswa->count() }}</span> dari <span class="font-medium text-[#4F8DB8]">{{ $totalSiswa }}</span> data
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <button class="w-8 h-8 rounded-lg bg-white border border-gray-200 hover:bg-[#4F8DB8] hover:text-white hover:border-[#4F8DB8] transition-all flex items-center justify-center text-gray-700">10</button>
            <button class="w-8 h-8 rounded-lg bg-white border border-gray-200 hover:bg-[#4F8DB8] hover:text-white hover:border-[#4F8DB8] transition-all flex items-center justify-center text-gray-500">
              <i class="fa-solid fa-chevron-right text-xs"></i>
            </button>
          </div>
  ...
  <script>
    ...
    // AUTO EXPAND DATA MASTER SUBMENU (karena halaman Data Siswa)
    document.addEventListener("DOMContentLoaded", function() {
      ...
      const dataMasterChevronMobile = document.getElementById('chevron-dataMasterMobile');
