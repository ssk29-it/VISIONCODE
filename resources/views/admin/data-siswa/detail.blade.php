...
<html lang="id">
  <head>
    <meta charset="UTF-8" />
    <title>Detail Siswa | BK Care SMK Budi Bakti Ciwidey</title>
    <meta
      name="viewport"
      ...
      <main class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 relative z-10">
        ...
        <div class="glass-card-deep rounded-xl md:rounded-2xl p-3 md:p-5 flex flex-wrap items-center justify-between gap-3 border border-white/40 shadow-xl">
          <div class="flex items-center gap-2 md:gap-3">
            ...
            <div>
              <h2 class="font-extrabold text-[#17235b] text-base md:text-xl">
                <span class="text-gradient">Detail Siswa</span>
              </h2>
              <p class="text-[10px] md:text-xs text-gray-500 flex items-center gap-1 md:gap-2">
                ...
                <span>Informasi lengkap data siswa</span>
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 text-xs text-gray-400 self-end sm:self-auto">
            ...
            <i class="fa-solid fa-chevron-right text-[10px]"></i>
            <span class="text-[#4F8DB8] font-medium">Detail Siswa</span>
          </div>
        </div>

        {{-- FORM DETAIL SISWA (READONLY) --}}
        <div class="glass-card-deep rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/40 shadow-xl max-w-4xl mx-auto">
          ...
          <div class="flex items-center gap-3 mb-6 pb-4 border-b border-[#4F8DB8]/20">
            ...
            <div>
              <h3 class="font-bold text-[#17235b]">Biodata Siswa</h3>
              <p class="text-xs text-gray-500">Data lengkap siswa</p>
            </div>
          </div>

          <!-- LAYOUT: TIDAK ADA FOTO (sesuai struktur database) -->
          <div class="flex flex-col gap-6">
            ...
            <div class="flex-1">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <div class="md:col-span-2">
                  <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">
                    <i class="fa-solid fa-chalkboard-user text-[#4F8DB8] mr-1"></i> Wali Kelas
                  </label>
                  <div class="relative">
                    <div class="absolute inset-0 bg-gradient-to-r from-[#4F8DB8] to-[#6FAED9] rounded-lg blur opacity-10"></div>
                    <input
                      type="text"
                      value="{{ $siswa->walikelas->nama_guru ?? 'Tidak ada wali kelas' }}"
                      readonly
                ...
                <div>
                  <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">
                    <i class="fa-regular fa-id-card text-[#4F8DB8] mr-1"></i> NIPD
                  </label>
                  <div class="relative">
                    <div class="absolute inset-0 bg-gradient-to-r from-[#4F8DB8] to-[#6FAED9] rounded-lg blur opacity-10"></div>
                    <input
                ...
                <div>
                  <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">
                    <i class="fa-regular fa-building text-[#4F8DB8] mr-1"></i> Kelas
                  </label>
                  <div class="relative">
                    <div class="absolute inset-0 bg-gradient-to-r from-[#4F8DB8] to-[#6FAED9] rounded-lg blur opacity-10"></div>
                    <input
                ...
                <div>
                  <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">
                    <i class="fa-regular fa-code text-[#4F8DB8] mr-1"></i> Jurusan
                  </label>
                  <div class="relative">
                    <div class="absolute inset-0 bg-gradient-to-r from-[#4F8DB8] to-[#6FAED9] rounded-lg blur opacity-10"></div>
                    <input
                      type="text"
                      value="{{ $siswa->jurusan == 'RPL' ? 'Rekayasa Perangkat Lunak' : ($siswa->jurusan == 'BDP' ? 'Bisnis Daring Pemasaran' : ($siswa->jurusan == 'DKV' ? 'Desain Komunikasi Visual' : 'Akuntansi')) }}"
                      readonly
                      class="form-input relative w-full h-10 px-4 rounded-lg border border-gray-200 bg-white/90 focus:outline-none focus:ring-2 focus:ring-[#4F8DB8] focus:border-transparent transition-all text-sm"
                    />
                  </div>
                ...
                <div>
                  ...
                  <div class="relative">
                    ...
                    <input
                      type="text"
                      value="{{ \Carbon\Carbon::parse($siswa->tanggal_lahir)->format('d F Y') }}"
                      readonly
                      class="form-input relative w-full h-10 px-4 rounded-lg border border-gray-200 bg-white/90 focus:outline-none focus:ring-2 focus:ring-[#4F8DB8] focus:border-transparent transition-all text-sm"
                    />
                  </div>
