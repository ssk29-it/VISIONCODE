...
<head>
  <meta charset="UTF-8" />
  <title>Data Wali Kelas | BK Care SMK Budi Bakti Ciwidey</title>
      ...
      <nav class="flex-1 overflow-y-auto py-5 space-y-1 text-sm">
        ...
        <div class="space-y-1">
          ...
          <div id="dataMasterMobile" class="ml-6 mt-1 space-y-1">
            ...
            <a href="{{  route('data-wali-kelas.index') }}" class="submenu-item flex items-center gap-3 px-4 py-2.5 rounded-xl bg-[#4F8DB8]/10 text-[#4F8DB8] font-medium pl-9">
              <span class="w-1 h-1 bg-[#4F8DB8] rounded-full"></span>
              <span class="text-sm">Data Wali Kelas</span>
              <span class="ml-auto bg-[#4F8DB8]/10 text-[#4F8DB8] text-[10px] px-1.5 py-0.5 rounded-full">{{ $totalWaliKelas }}</span>
            </a>
            <a href="{{ route('data-siswa.index') }}" class="submenu-item flex items-center gap-3 px-4 py-2.5 rounded-xl text-gray-600 hover:bg-[#4F8DB8]/10 transition-all pl-9">
              <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
    ...
    <aside class="hidden lg:block sidebar-desktop z-10">
      <div class="inner-sidebar glass-card-deep rounded-2xl m-4 p-5 w-80 border border-white/40 shadow-2xl">
        ...
        <nav class="flex-1 overflow-y-auto py-5 space-y-1 text-sm">
          ...
          <!-- Data Master - ACTIVE -->
          <div class="space-y-1">
            <bu tton onclick="toggleMenu('dataMaster')" 
              class="menu-item w-full flex items-center justify-between px-4 py-3 rounded-xl text-gray-700 hover:bg-[#4F8DB8]/10 transition-all menu-active">
              <div class="flex items-center gap-3">
                <i class="fa-solid fa-database w-5 text-center text-white"></i>
                <span class="font-medium text-white">Data Master</span>
    ...
    <main class="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 relative z-10 lg:ml-0">
      ...
      <div class="glass-card-deep rounded-xl md:rounded-2xl p-3 md:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-white/40 shadow-xl">
        <div class="flex items-center gap-2 md:gap-3">
          ...
          <div>
            ...
            <p class="text-[10px] md:text-xs text-gray-500 flex items-center gap-1 md:gap-2">
              ...
              <span>Data Wali Kelas SMK Budi Bakti Ciwidey</span>
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 text-xs text-gray-400 self-end sm:self-auto">
          ...
          <i class="fa-solid fa-chevron-right text-[10px]"></i>
          <span class="text-[#4F8DB8] font-medium">Data Wali Kelas</span>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div class="glass-card-deep rounded-xl p-4 flex-1 flex items-center gap-4 border border-white/40 shadow-md">
          <div class="w-12 h-12 bg-[#6FAED9]/10 rounded-xl flex items-center justify-center">
            <i class="fa-solid fa-chalkboard-user text-[#6FAED9] text-xl"></i>
          </div>
          <div>
            <h3 class="font-semibold text-[#17235b]">Total Wali Kelas Aktif</h3>
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold text-[#17235b]">{{ $totalWaliKelas }}</span>
              <span class="text-xs text-gray-500">wali kelas</span>
              <span class="ml-2 text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">12 kelas</span>
            </div>
          </div>
        </div>

        <!-- BUTTON TAMBAH -->
        <a href="{{ route('data-wali-kelas.create') }}" class="flex items-center gap-2 px-6 py-3.5 bg-gradient-to-r from-[#4F8DB8] to-[#6FAED9] text-white text-sm font-semibold rounded-xl hover:shadow-lg hover:shadow-[#4F8DB8]/30 transition-all duration-300 hover:scale-[1.02] shadow-md">
...
<div class="glass-card-deep rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/40 shadow-xl">
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          {{-- search --}}
      <form method="GET" action="{{ route('data-wali-kelas.index') }}" class="relative flex-1 max-w-md">
    ...
    <!-- FILTER BUTTON -->
<form method="GET" action="{{ route('data-wali-kelas.index') }}" class="flex items-center gap-3">
    ...
    <table class="w-full text-sm border-separate border-spacing-0">
      <thead>
        <tr class="bg-gradient-to-r from-[#4F8DB8]/10 to-[#6FAED9]/10">
          ...
          <th class="px-4 py-3 text-left text-xs font-semibold text-[#17235b] uppercase tracking-wider">Nama Guru</th>
          <th class="px-4 py-3 text-left text-xs font-semibold text-[#17235b] uppercase tracking-wider">NIP</th>
      ...
      <tbody class="divide-y divide-gray-200">
        ...
        <tr class="hover:bg-[#4F8DB8]/5 transition-colors group">
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <span class="font-medium text-gray-800">{{ $item->nama_guru }}</span>
            </div>
          </td>
          <td class="px-4 py-3 text-sm text-gray-600 font-mono">{{ $item->nip }}</td>
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              ...
              {{-- detail --}}
            <a href="{{ route('data-wali-kelas.show', $item->id) }}"
              ...
              <i class="fa-regular fa-eye mr-1"></i> Detail
                ...
                {{-- edit --}}
            <a href="{{ route('data-wali-kelas.edit', $item->id) }}"
            ...
            <i class="fa-regular fa-pen-to-square"></i>
              ...
              <form id="delete-form-{{ $item->id }}"
                    action="{{ route('data-wali-kelas.destroy', $item->id) }}"
                    method="POST"
                    class="inline">
                  @csrf
                  @method('DELETE')
  ...
  <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-4 border-t border-[#4F8DB8]/20">
    ...
    <div class="text-sm text-gray-500">
      Menampilkan <span class="font-medium text-[#4F8DB8]">1-{{ $walikelas->count() }}</span> dari <span class="font-medium text-[#4F8DB8]">{{ $totalWaliKelas }}</span> data
    </div>
  ...
  <script>
    ...
    // AUTO EXPAND DATA MASTER SUBMENU (karena halaman Data Wali Kelas)
